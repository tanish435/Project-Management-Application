...
export async function PATCH(req: Request, context: { params: Promise<{ cardId: string, attachmentId: string }> }) {
    await dbConnect()
    ...
    try {
        ...
        const updateName = await AttachmentModel.findByIdAndUpdate(
            attachmentId,
            {
                $set: { name }
            },
        ...
        if (!updateName) {
            const errResponse = new ApiResponse(404, null, "Attachment not found");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const successResponse = new ApiResponse(200, updateName, "Attachment display name updated successfully");
        return new Response(JSON.stringify(successResponse), {
            status: successResponse.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error updating attachment display name:", error);
