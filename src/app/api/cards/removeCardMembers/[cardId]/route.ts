...
export async function PATCH(req: Request, { params }: { params: { cardId: string } }) {
    ...
    try {
        ...
        const removeMember = await CardModel.findByIdAndUpdate(
            cardId,
            {
        ...
        if(!removeMember) {
            const errResponse = new ApiResponse(400, null, "Failed to remove member from card");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const successResponse = new ApiResponse(200, removeMember, "Member removed from card successfully");
        return new Response(JSON.stringify(successResponse), {
            status: successResponse.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error removing members from card", error);
