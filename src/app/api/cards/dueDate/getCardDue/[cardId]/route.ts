...
export async function GET(req: Request, context: { params: Promise<{ cardId: string }> }) {
    ...
    try {
        ...
        const card = await CardModel.findById(cardId).select({_id: 1, dueDate: 1})

        if(!card) {
            const errResponse = new ApiResponse(404, null, "Card due details not found");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const response = new ApiResponse(200, card, "Card due date fetched successfully");
        return new Response(JSON.stringify(response), {
            status: response.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error fetching card due", error);
