...
export async function PATCH(req: Request, {params}: {params: {cardId: string}}) {
    ...
    try {
        ...
        if(!card) {
            const errResponse = new ApiResponse(404, null, "Failed to remove card due");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const response = new ApiResponse(200, card, "Card due date removed successfully");
        return new Response(JSON.stringify(response), {
            status: response.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error removing card due", error);
