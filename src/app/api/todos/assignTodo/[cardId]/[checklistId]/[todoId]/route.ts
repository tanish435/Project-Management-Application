...
export async function PATCH(req: Request, context: { params: Promise<{ cardId: string, checklistId: string, todoId: string }> }) {
    ...
    try {
        ...
        const assignTodo = await TodoModel.findOneAndUpdate(
        ...
        if(!assignTodo) {
            const errResponse = new ApiResponse(400, null, "Failed to assign todo");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const successResponse = new ApiResponse(200, assignTodo, "Todo assigned successfully");
