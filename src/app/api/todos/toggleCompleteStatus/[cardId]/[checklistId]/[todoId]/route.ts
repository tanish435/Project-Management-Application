...
export async function PATCH(req: Request, { params }: { params: { cardId: string, checklistId: string, todoId: string } }) {
    ...
    try {
        ...
        if (!todo?.complete) {
            const toggleTodo = await TodoModel.findOneAndUpdate(
                {_id: todoId, checklist: checklistId},
                {
                    $set: { complete: true }
                },
                { new: true }
            ...
            if (!toggleTodo) {
                const errResponse = new ApiResponse(400, null, "Failed to mark todo complete");
                return new Response(JSON.stringify(errResponse), {
                    status: errResponse.statusCode,
                    headers: { "Content-Type": "application/json" },
                });
            ...
            const successResponse = new ApiResponse(200, toggleTodo, "Todo marked complete");
            ...
            if (!toggleTodo) {
                const errResponse = new ApiResponse(400, null, "Failed to mark todo incomplete");
                return new Response(JSON.stringify(errResponse), {
                    status: errResponse.statusCode,
                    headers: { "Content-Type": "application/json" },
                });
            ...
            const successResponse = new ApiResponse(200, toggleTodo, "Todo marked incomplete");
            return new Response(JSON.stringify(successResponse), {
                status: successResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        console.log("Error toggling todo complete status", error);
