...
export async function PATCH(req: Request, context: { params: Promise<{ cardId: string, checklistId: string, todoId: string }> }) {
    ...
    const { cardId, checklistId, todoId } = await context.params
    ...
    try {
        ...
        const updateContent = await TodoModel.findOneAndUpdate(
        ...
        console.log(updateContent, "check");
        

        if(!updateContent) {
            const errResponse = new ApiResponse(400, null, "Failed to update content");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const successResponse = new ApiResponse(200, updateContent, "Todo content updated successfully");
        return new Response(JSON.stringify(successResponse), {
            status: successResponse.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error updating todo content", error);
