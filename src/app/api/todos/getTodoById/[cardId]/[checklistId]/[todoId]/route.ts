...
export async function GET(req: Request, context: { params: Promise<{ cardId: string, checklistId: string, todoId: string }> }) {
    await dbConnect()
    ...
    try {
        ...
        const todos = await TodoModel.aggregate([
            {
                $match: {_id: new mongoose.Types.ObjectId(todoId), checklist: new mongoose.Types.ObjectId(checklistId)}
            },
            {
                $sort: {pos: 1}
            },
        ...
        if (!todos || todos.length === 0) {
            const errResponse = new ApiResponse(200, null, "Todos not found");
            return new Response(JSON.stringify(errResponse), {
                status: errResponse.statusCode,
                headers: { "Content-Type": "application/json" },
            });
        ...
        const successResponse = new ApiResponse(200, todos[0], "Fethced todo by Id successfully");
        return new Response(JSON.stringify(successResponse), {
            status: successResponse.statusCode,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error) {
        console.log("Error fetching checklist todos", error);
